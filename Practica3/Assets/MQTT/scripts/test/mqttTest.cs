...
public class mqttTest : MonoBehaviour {
	...
	void Start () {
		...
		client.Subscribe(new string[] { temperatureTopic,lightTopic  }, new byte[] { MqttMsgBase.QOS_LEVEL_EXACTLY_ONCE });
	...
	void OnGUI(){
		if ( GUI.Button (new Rect (20,40,80,20), "Encender Luz")) {
			Debug.Log("sending...");
			client.Publish("casa/sala/luz", System.Text.Encoding.UTF8.GetBytes("lightOn"), MqttMsgBase.QOS_LEVEL_EXACTLY_ONCE, true);
			Debug.Log("sent");
		}

		if ( GUI.Button (new Rect (40,60,100,40), "Apagar Luz")) {
			Debug.Log("sending...");
			client.Publish("casa/sala/luz", System.Text.Encoding.UTF8.GetBytes("lightOff"), MqttMsgBase.QOS_LEVEL_EXACTLY_ONCE, true);
			Debug.Log("sent");
		}
	}
